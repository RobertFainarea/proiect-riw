...
const buildIndexes = (inputPath, directIndexPath, indirectIndexPath) => {
  ...
  fs.writeFileSync(directIndexPath, JSON.stringify(dirIndexDocs, null, 4));

  const indIndexDocs = buildIndirectIndex(dirIndexDocs);
  fs.writeFileSync(indirectIndexPath, JSON.stringify(indIndexDocs, null, 4));
};
